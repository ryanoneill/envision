...
impl Component for FileBrowser {
    ...
    fn view(state: &Self::State, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let path_style = if state.internal_focus == FileBrowserFocus::PathBar && state.focused {
            theme.focused_style()
        } else {
            theme.info_style()
        };
        ...
        let list_style = if state.internal_focus == FileBrowserFocus::FileList && state.focused {
            theme.focused_style()
        } else {
            theme.normal_style()
        };

        let highlight_style = theme.selected_style(state.focused);
