...
#[test]
fn test_with_warning_threshold() {
    let w = MetricWidget::gauge("CPU", 60, 100).with_warning_threshold(0.5);
    assert_eq!(w.warning_threshold(), Some(0.5));
}

#[test]
fn test_with_critical_threshold() {
    let w = MetricWidget::gauge("CPU", 95, 100).with_critical_threshold(0.8);
    assert_eq!(w.critical_threshold(), Some(0.8));
}

#[test]
fn test_default_thresholds_none() {
    let w = MetricWidget::gauge("CPU", 50, 100);
    assert_eq!(w.warning_threshold(), None);
    assert_eq!(w.critical_threshold(), None);
}

#[test]
fn test_effective_warning_threshold_default() {
    let w = MetricWidget::gauge("CPU", 50, 100);
    assert!((w.effective_warning_threshold() - 0.7).abs() < f64::EPSILON);
}

#[test]
fn test_effective_warning_threshold_custom() {
    let w = MetricWidget::gauge("CPU", 50, 100).with_warning_threshold(0.5);
    assert!((w.effective_warning_threshold() - 0.5).abs() < f64::EPSILON);
}

#[test]
fn test_effective_critical_threshold_default() {
    let w = MetricWidget::gauge("CPU", 50, 100);
    assert!((w.effective_critical_threshold() - 0.9).abs() < f64::EPSILON);
}

#[test]
fn test_effective_critical_threshold_custom() {
    let w = MetricWidget::gauge("CPU", 50, 100).with_critical_threshold(0.8);
    assert!((w.effective_critical_threshold() - 0.8).abs() < f64::EPSILON);
}
...
#[test]
fn test_with_units() {
    let w = MetricWidget::gauge("Memory", 512, 1024).with_units("MB");
    assert_eq!(w.units(), Some("MB"));
    assert_eq!(w.display_value(), "512/1024 MB");
}

#[test]
fn test_counter_with_units() {
    let w = MetricWidget::counter("Requests", 42).with_units("req/s");
    assert_eq!(w.display_value(), "42 req/s");
}

#[test]
fn test_no_units_by_default() {
    let w = MetricWidget::counter("A", 42);
...
#[test]
fn test_set_value_with_previous_increase() {
    let mut w = MetricWidget::counter("Requests", 100);
    w.set_value_with_previous(150.0, 100.0);
    assert_eq!(w.previous_value(), Some(100.0));
    let display = w.display_value();
    assert!(display.contains("\u{2191}")); // up arrow
    assert!(display.contains("50.0%"));
}

#[test]
fn test_set_value_with_previous_decrease() {
    let mut w = MetricWidget::counter("Requests", 100);
    w.set_value_with_previous(80.0, 100.0);
    let display = w.display_value();
    assert!(display.contains("\u{2193}")); // down arrow
    assert!(display.contains("20.0%"));
}

#[test]
fn test_set_value_with_previous_no_change() {
    let mut w = MetricWidget::counter("Requests", 100);
    w.set_value_with_previous(100.0, 100.0);
    assert_eq!(w.display_value(), "100");
}

#[test]
fn test_set_value_with_previous_from_zero() {
    let mut w = MetricWidget::counter("Requests", 0);
    w.set_value_with_previous(50.0, 0.0);
    let display = w.display_value();
    assert!(display.contains("\u{2191}"));
    assert!(display.contains("50.0")); // absolute, not percentage
}

#[test]
fn test_previous_value_default_none() {
    let w = MetricWidget::counter("A", 42);
    assert_eq!(w.previous_value(), None);
}

#[test]
fn test_set_value_with_previous_gauge() {
    let mut w = MetricWidget::gauge("CPU", 50, 100);
    w.set_value_with_previous(75.0, 50.0);
    assert_eq!(w.previous_value(), Some(50.0));
    let display = w.display_value();
    assert!(display.contains("\u{2191}"));
    assert!(display.contains("50.0%"));
}
...
#[test]
fn test_units_and_delta_together() {
    let mut w = MetricWidget::counter("Requests", 100).with_units("req/s");
    w.set_value_with_previous(150.0, 100.0);
    let display = w.display_value();
    assert!(display.contains("req/s"));
    assert!(display.contains("\u{2191}"));
}
...
fn test_gauge_bar_half_fill() {
    let w = MetricWidget::gauge("CPU", 50, 100);
    let bar = w.gauge_bar(10).unwrap();
    assert_eq!(bar.chars().count(), 10);
}

#[test]
fn test_gauge_bar_full_fill() {
    let w = MetricWidget::gauge("CPU", 100, 100);
    let bar = w.gauge_bar(10).unwrap();
    // All full blocks
    for ch in bar.chars() {
        assert_eq!(ch, '\u{2588}');
...
fn test_gauge_bar_zero_fill() {
    let w = MetricWidget::gauge("CPU", 0, 100);
    let bar = w.gauge_bar(10).unwrap();
    assert_eq!(bar.chars().count(), 10);
    // First char should be space (zero fractional), rest spaces
    for ch in bar.chars() {
        assert_eq!(ch, ' ');
    }
}
...
fn test_gauge_bar_zero_width() {
    let w = MetricWidget::gauge("CPU", 50, 100);
    assert!(w.gauge_bar(0).is_none());
}

#[test]
fn test_gauge_bar_non_gauge_returns_none() {
    let w = MetricWidget::counter("Count", 50);
    assert!(w.gauge_bar(10).is_none());
}
...
#[test]
fn test_render_with_gauge_bar() {
    let state = MetricsDashboardState::new(vec![MetricWidget::gauge("CPU %", 75, 100)], 1);
...
fn test_render_with_units() {
    let state = MetricsDashboardState::new(
        vec![MetricWidget::gauge("Memory", 512, 1024).with_units("MB")],
        1,
    );
    let (mut terminal, theme) = test_utils::setup_render(40, 10);
    terminal
...
#[test]
fn test_render_with_custom_thresholds() {
    let state = MetricsDashboardState::new(
        vec![MetricWidget::gauge("CPU", 55, 100)
...
#[test]
fn test_render_with_delta() {
    let mut w = MetricWidget::counter("Requests", 100);
    w.set_value_with_previous(150.0, 100.0);
    let state = MetricsDashboardState::new(vec![w], 1);
    let (mut terminal, theme) = test_utils::setup_render(40, 10);
    terminal
        .draw(|frame| {
            MetricsDashboard::view(&state, frame, frame.area(), &theme);
